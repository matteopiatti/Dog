...
def select_split_action(action: Action, allowed_steps: dict) -> None:
    print("\033[s")
    print(
        "\033[1;50H\033[4mSplit Action Selected. Allowed steps for each marble:\033[0m"
    )
    options = []
    i = 0
...
def select_switch_card(state) -> tuple:
    while True:
        choice = input("Select a card to give to teammate: ")
        if choice.isdigit():
            choice_idx = int(choice) - 1
            if 0 <= choice_idx < len(state.current_player.hand):
                selected_card = state.current_player.hand[choice_idx]
                return selected_card
        error_print("Invalid choice. Please try again.")


def print_switch(state) -> None:
    print("\033[s")
    print("\033[1;50H\033[4mSwitch Phase. Choose a card to give to teammate.\033[0m")
    for idx, card in enumerate(state.current_player.hand):
        card_str = get_printable_card(card)
        print(f"\033[{idx+2};50H{idx + 1}:  {card_str}")
    print("\033[u")
