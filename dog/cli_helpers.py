...
def cell_str(state, i):
    if state.board.track[i] == (None, None):
        if i in state.board.start_fields.values():
            color = next(
                pl.color.value for pl, sf in state.board.start_fields.items() if sf == i
            )
            return color + "◯" + "\033[0m"
        return "."
    else:
...
def print_action_line(action, idx):
    ...
    swap_marble = (
        get_printable_marble(action.swap_marble, action.swap_player)
        if action.swap_marble
        else ""
    )
    ...
    print(
        f"\033[{idx+2};50H  {idx + 1}: {card} - {marble} {swap_marble} {' - ' + steps if steps else ''}\033[0m"
    )
